"""
usage/scanner.py - Scans Claude Code JSONL transcript files and stores data in SQLite.

Sourced from github.com/phuryn/claude-usage (MIT License) and integrated into
ClaudeClaw OS for cost tracking.

Usage:
  python usage/scanner.py
  python usage/scanner.py --projects-dir /custom/path
"""
...
def scan(projects_dir=None, projects_dirs=None, db_path=DB_PATH, verbose=True):
    ...
    if verbose:
        print(f"\nScan complete: new={new_files} updated={updated_files} skipped={skipped_files} turns={total_turns} sessions={len(total_sessions)}")
